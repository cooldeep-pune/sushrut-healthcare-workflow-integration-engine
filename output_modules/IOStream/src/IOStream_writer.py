...
def push(
    ...
    *values,
    ...
    try:
        ...
        print(queue_name)
        redis_cli_output.rpush(queue_name,*values)
...
def run_reader():
    ...
    redis_cli_input = Redis(host=os.environ['REDIS_HOST'], port=int(os.environ['REDIS_PORT']), db=0)
    redis_cli_output = Redis(host=hostname, port=int(port), db=0)

    while True:
        msg = redis_cli_input.rpop(queue_name)
        if msg is None:
            time.sleep(0.1)
            continue
        try:
            push(dest_queue_name,msg,redis_cli_output)
