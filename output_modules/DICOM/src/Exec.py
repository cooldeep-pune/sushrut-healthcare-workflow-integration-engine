...
def run_connector(
    ...
    """ staring output connector """
    destinationDockerContainer = json_channel['channel']['destinationConnectors']['connector']['properties']['@class']
    destinationDockerContainerName = destinationDockerContainer.replace('/','-')
    print('starting '+destinationDockerContainerName)

    dest_port = json_channel['channel']['destinationConnectors']['connector']['properties']['port']
    dest_ae_title = json_channel['channel']['destinationConnectors']['connector']['properties']['applicationEntity']
    dest_host = json_channel['channel']['destinationConnectors']['connector']['properties']['host']
    
    os.system("python3 /opt/healthcare-workflow/healthcare-workflow-DICOMSender.py -aet " + dest_ae_title + " " + dest_host + " " + dest_port + " -c " + channel_id)
